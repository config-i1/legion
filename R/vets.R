...
vets <- function(data, model="PPP", lags=c(frequency(data)),
    ...
    fillerVETS <- function(matVt, matF, matG, matW, B,
        ...
        if(componentsCommonLevel){
            ...
            matG[cbind(1:nComponentsLevel,1:nSeries)] <- B[1:nParametersLevel];
        }
        ...
        if(modelIsTrendy){
            if(componentsCommonTrend){
                ...
                matG[cbind(k+1:nComponentsTrend,1:nSeries)] <- B[j+1:nParametersTrend];
            }
        ...
        if(modelIsSeasonal){
            if(componentsCommonSeasonal){
                ...
                matG[cbind(k+1:nComponentsSeasonal,1:nSeries)] <- B[j+1:nParametersSeasonal];
            }
        ...
        if(modelIsTrendy && damped){
            ...
            if(componentsCommonTrend){
                ...
                if(componentsCommonLevel){
                    ...
                    # matF[1:nComponentsLevel,nComponentsLevel+1:nComponentsTrend] <-
                    #     B[j+1:nParametersDamped]*diag(nComponentsTrend);
                    matF[cbind(1:nComponentsLevel,nComponentsLevel+1:nComponentsTrend)] <-
                        B[j+1:nParametersDamped];
                }
                # Damped parameter in the trend equations
                # matF[nComponentsLevel+1:nComponentsTrend,nComponentsLevel+1:nComponentsTrend] <-
                #     B[j+1:nParametersDamped]*diag(nComponentsTrend);
                matF[cbind(nComponentsLevel+1:nComponentsTrend,nComponentsLevel+1:nComponentsTrend)] <-
                    B[j+1:nParametersDamped];
            }
            ...
            if(componentsCommonTrend){
                ...
                matW[cbind(1:nSeries,nComponentsLevel+1:nSeries)] <- B[j+1:nParametersDamped];
            }
    ...
    CF <- function(B, loss="likelihood", Etype="A"){
        ...
        fitting <- vFitterWrap(switch(Etype, "M"=log(yInSample), yInSample),
                               ...
                               initialType=="backcasting", nComponentsTrend);
